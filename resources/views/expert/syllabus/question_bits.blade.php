    ...
    <div class="bg-white p-6 rounded-xl shadow">
        ...
        <form method="POST" action="{{ route('expert.syllabus.qb.save', $course->id) }}">
            ...
            <div class="overflow-x-auto">
                <table class="border border-gray-300 text-sm text-center" >
                ...
                <tbody class="divide-y">
                    ...
                    @foreach ($qppRows as $row)
                        ...
                        @foreach ($bits_list as $bIdx => $bit)
                            <tr class="hover:bg-gray-50" data-unit="{{ $unitId }}" data-bit="{{ $bit }}">
                                ...
                                @if ($loop->first)
                                    <td rowspan="8" class="border px-2 py-2 text-gray-800 font-semibold bg-yellow-50">
                                        {{ $actualDist }}
                                    </td>
                                @endif
                        ...
                        <tr class="bg-blue-50 font-semibold" data-unit-subtotal="{{ $unitId }}">
                            ...
                            @for ($q = 1; $q <= 6; $q++)
                                <td class="border px-2 py-2 q-subtotal font-semibold" data-unit="{{ $unitId }}"
                                    data-q="{{ $q }}" data-expected="{{ $qExpected[$q] }}">
                                    0
                                </td>
                            @endfor
