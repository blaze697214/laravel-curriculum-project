    ...
    <form method="POST" action="{{ route('cdc.schemes.update', $scheme->id) }}" class="space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <select name="year_start" id="yearStart" required
                    ...
                    @for ($y = date('Y'); $y <= date('Y') + 5; $y++)
                        <option value="{{ $y }}" {{ old('year_start',$scheme->year_start) == $y ? 'selected' : '' }}>
                            {{ $y }}
                        </option>
                    @endfor
            ...
            <div>
                ...
                <input name="year_end" type="text" id="yearEnd" readonly value="{{ old('year_end', $scheme->year_end) }}"
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="number" name="total_credits" value="{{ old('total_credits', $scheme->total_credits) }}"
                    class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>

            <div>
                <label class="block text-sm text-gray-600 mb-1">Total Marks</label>
                <input type="number" name="total_marks" value="{{ old('total_marks', $scheme->total_marks) }}"
                    class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
