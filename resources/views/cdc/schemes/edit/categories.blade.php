...
@section('content')
    ...
    <div class="flex gap-7">
        ...
        <div class="bg-white p-6 rounded-xl shadow basis-1/3">
            ...
            <form method="POST" action="{{ route('cdc.schemes.edit.categories.store', $scheme->id) }}" class="space-y-4">
                @csrf
                    ...
                    <tbody class="divide-y">
                        ...
                        @forelse($categories as $category)
                            <tr class="hover:bg-gray-50 border-gray-200">
                                ...
                                <td class="px-4 py-3">
                                    <form method="POST"
                                        action="{{ route('cdc.schemes.edit.categories.update', $category->id) }}"
                                        class="flex gap-2">
                                        @csrf
                                        @method('PATCH')
                                ...
                                <td class="px-4 py-3 flex gap-2">
                                    ...
                                    <form method="POST"
                                        action="{{ route('cdc.schemes.edit.categories.destroy', $category->id) }}">
                                        @csrf
                                        @method('DELETE')
    ...
    <div class="mt-8 flex justify-between">
        <a href="{{ route('cdc.schemes.edit', $scheme->id) }}">
            <button class="bg-gray-500 hover:bg-gray-600 text-white px-5 py-2 rounded-lg transition w-40">
                ← Back
            </button>
        </a>
        <form method="POST" action="{{ route('cdc.schemes.edit.categories.next', $scheme->id) }}">
            @csrf

            <button type="submit" class="bg-green-600 hover:bg-green-700 text-white px-5 py-2 rounded-lg transition w-40">
                Next →
            </button>
        </form>
