...
@section('content')
                        ...
                        @foreach ($semCourses as $i => $c)
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-2">
                                    ...
                                    <form method="POST" action="{{ route('hod.assign.store') }}"
                                ...
                                <td class="px-4 py-2">
                                    <select name="moderator_id"  class="border border-gray-300 w-full rounded px-2 py-1 text-sm focus:ring-2 focus:ring-blue-500"
                                            required>
                                                                                <option value="">Select Moderator</option>

                                        @foreach ($moderator as $m)
                                            <option value="{{ $m->id }}"
                                                {{ $assignment && $assignment->moderator_id == $m->id ? 'selected' : '' }}>
                                                {{ $m->name }}
                                            </option>
                                        @endforeach
    ...
    <div class="bg-white p-6 rounded-xl shadow">
        ...
        <h2 class="text-lg font-semibold mb-4">Moderator-wise Assigned Courses</h2>
        ...
        <div class="overflow-x-auto rounded-xl shadow bg-white">
            <table class="w-full text-sm border border-gray-200 text-center">
                ...
                <thead class="bg-gray-100 text-gray-700">
                    <tr>
                        <th class="px-4 py-2 text-left">Moderator</th>
                        <th class="px-4 py-2">Courses</th>
                    </tr>
                </thead>
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    @foreach ($moderatorAssignments as $expertId => $items)
                        <tr class="hover:bg-gray-50">
